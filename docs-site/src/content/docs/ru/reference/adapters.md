...
с последующим одним потоком ответа. Затем [`bridge.ts`](/opencodex-universal/ru/reference/architecture/#мост)
...
- В режиме `forward` ретранслируется только безопасный allowlist заголовков (`FORWARD_HEADERS`):
  ...
  через ChatGPT, на котором также работают [сайдкары](/opencodex-universal/ru/guides/sidecars/).
