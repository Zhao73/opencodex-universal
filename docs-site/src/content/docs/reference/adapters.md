...
HTTP fetch followed by one response stream. [`bridge.ts`](/opencodex-universal/reference/architecture/#the-bridge)
...
- In `forward` mode only a safe header allowlist is relayed (`FORWARD_HEADERS`): authorization,
  ...
  that also powers the [sidecars](/opencodex-universal/guides/sidecars/).
