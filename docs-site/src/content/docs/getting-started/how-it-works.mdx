...
2. **Route** — `router.ts` maps the requested model id to a configured provider using a fixed
   ...
   the `defaultProvider` fallback. See [Model Routing](/opencodex-universal/guides/model-routing/).
...
4. **Vision sidecar (optional)** — if the routed model is listed in `provider.noVisionModels` and the
   ...
   See [Sidecars](/opencodex-universal/guides/sidecars/).
...
all round-trip correctly. See the [Architecture reference](/opencodex-universal/reference/architecture/) for the
