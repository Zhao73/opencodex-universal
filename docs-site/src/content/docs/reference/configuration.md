...
| `authMode?` | `"key" \| "forward" \| "oauth"` | How to authenticate (default `key`). See [Providers](/opencodex-universal/guides/providers/#auth-modes). |
...
| `noVisionModels?` | `string[]` | Text-only models — the [vision sidecar](/opencodex-universal/guides/sidecars/) describes images for them. Matching tolerates an Ollama `:size` tag. |
...
You can also set it from the [web dashboard](/opencodex-universal/guides/web-dashboard/): **Providers →
