...
새로 설치하면 `subagentModels` 기본값으로 `gpt-5.5`, GPT-5.6 Sol/Terra/Luna 세 모델,
`gpt-5.4-mini`가 Codex의 sub-agent 선택기에 표시됩니다. 대시보드에서 네이티브 모델과 라우팅
...
2. **Route** — `router.ts`가 고정된 우선순위에 따라 요청된 모델 id를 설정된 프로바이더로
   ...
   `defaultProvider` 폴백. [모델 라우팅](/opencodex-universal/ko/guides/model-routing/)을 참고하세요.
...
4. **Vision sidecar(선택)** — 라우팅된 모델이 `provider.noVisionModels`에 나열되어 있고
   ...
   [Sidecar](/opencodex-universal/ko/guides/sidecars/)를 참고하세요.
...
`tool_search` 발견이 모두 정확히 왕복합니다. 이벤트별 매핑은 [아키텍처 레퍼런스](/opencodex-universal/ko/reference/architecture/)를
참고하세요.
