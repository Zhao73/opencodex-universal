...
| **Codex 自動起動** | Codex CLI/App 実行前に `ocx ensure` を呼ぶランチャーシムをオン/オフします。 |
...
[サブエージェントサーフェス](/opencodex-universal/ja/guides/sub-agent-surface/)を参照してください。
:::
...
- Thread affinity がリクエストごとにアカウントが揺れるのを防ぎます。クォータ自動切り替えがオンなら長く
  実行される thread も定期的に再評価します。関連使用量がしきい値を超え、使用量が確実により低い
...
| `GET /api/codex-auth/accounts?refresh=1` | メインおよびプールアカウントを参照しクォータを強制更新します。 |
...
[ビジョンサイドカー](/opencodex-universal/ja/guides/sidecars/)が正しい条件でのみ実行されます。
:::
