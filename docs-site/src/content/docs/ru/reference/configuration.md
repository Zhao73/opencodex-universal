...
| `authMode?` | `"key" \| "forward" \| "oauth"` | Способ аутентификации (по умолчанию `key`). См. [Провайдеры](/opencodex-universal/ru/guides/providers/#режимы-аутентификации). |
...
| `noVisionModels?` | `string[]` | Модели только для текста — [vision-сайдкар](/opencodex-universal/ru/guides/sidecars/) описывает для них изображения. Сопоставление допускает тег Ollama `:size`. |
...
Его также можно установить из [веб-дашборда](/opencodex-universal/ru/guides/web-dashboard/): **Providers →
